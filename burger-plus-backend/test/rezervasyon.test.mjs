...
import { rezervasyonTablosunuHazirla, rezervasyonVerisiniDogrula } from "../rezervasyonDb.js";
...
test("rezervasyon migrationi PostgreSQL anahtar kelimesini sütun adı olarak kullanmaz", async () => {
  let sorgu = "";
  await rezervasyonTablosunuHazirla({ query: async (sql) => { sorgu = sql; return { rows: [] }; } });
  assert.match(sorgu, /notlar TEXT/);
  assert.doesNotMatch(sorgu, /,\s*not\s+TEXT/i);
});
